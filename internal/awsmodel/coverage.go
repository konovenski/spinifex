...
func RenderInternalReport(coverages []OperationCoverage) string {
	var report strings.Builder
	...
	for _, coverage := range sortedCoverages(coverages) {
		...
		fmt.Fprintf(&report, "`%s` — %d of %d modelled operations implemented (%.1f%%), %d stubbed, %d unsupported, %d registered outside the pinned model.\n\n",
			coverage.APIVersion, len(coverage.Implemented), len(coverage.Modelled), coverage.ImplementedPercent(),
			len(coverage.Stubbed), len(coverage.Unsupported), len(coverage.Extra))
		writeOperationList(&report, "Not implemented", coverage.Missing)
		writeOperationList(&report, "Stubbed", coverage.Stubbed)
		writeOperationList(&report, "Registered and refused", coverage.Unsupported)
